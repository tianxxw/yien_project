...
# 执行插入语句
sql="INSERT INTO yiendata7(Irank,ReleaseTime,iCount,MovieSumBoxOffice,BoxOffice_Pro,MovieName,EnMovieID,ReleaseDays," \
...
url='https://ys.endata.cn/enlib-api/api/home/getrank_moviecount.do?r=0.5770351455605036&top=50'
...
for item in data1:
    ...
    iCount=item['iCount']
    MovieSumBoxOffice= item['MovieSumBoxOffice']
    BoxOffice_Pro = item['BoxOffice_Pro']
    ...
    cursor.execute(sql,[str(Irank),str(ReleaseTime),str(iCount),str(MovieSumBoxOffice),str(BoxOffice_Pro),str(MovieName),str(EnMovieID),
